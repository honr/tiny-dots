...
// Destructively split string into a str_list.  `str' cannot be const (or
...
// Split buf of maximum size buf_size into a str_list.  Empty terms are not
// allowed (terminates the str_list at that point).  Quotations (single and
...
// Place the str_list lst in the buffer. Places a NULL character after each
...
// Create a str_list from a pack in the input buffer.  buf_cur will be updated
