...
// TODO: daemon mode should actually fork and become daemon. use syslog for
// logs.
...
int usage (int n) {
  printf (
      "usage:\n"
      " clove-daemon [--force-wipe] [--extraenv <extra env>] [--runpath "
      "<run path>] [--foreground]\n"
      " clove-daemon --help\n"
...
int main (int argc, char* argv[], char** envp) {
  static struct option long_options[] = {{"foreground", 0, 0, 'D'},
                                         {"extraenv", 1, 0, 'x'},
                                         {"runpath", 1, 0, 'p'},
                                         {"force-wipe", 0, 0, 'w'},
                                         {"version", 0, 0, 'v'},
  ...
  while (((c = getopt_long (argc, argv, "x:p:wDhv", long_options,
                            &option_index)) != -1)) {
    switch (c) {
      case 'x':
  ...
  while (running) {
    ...
    if ((client_handler_pid = fork ()) == 0) {
      ...
      if (buf[0] == 0)  // remote command
      {
        ...
        // TODO: see what happens to process group when the parent exits.  At
        //   the beginning read about former processes and a killer for each
        //   to the specific signal handler. Add the option -Q to kill all
        //   current and former children.

        struct service srv = service_init (service_name);

        // TODO: Check if service listens to its socket.  Find a better way to
