...
int usage (int n) {
  printf (
      "usage:\n"
      " clove [--force-wipe|--extraenv <extra env>|--runpath <run path>] <rest "
      "args>\n"
...
int main (int argc, char* argv[], char** envp) {
  static struct option long_options[] = {{"list", 0, 0, 'l'},
                                         {"extraenv", 1, 0, 'x'},
                                         {"runpath", 1, 0, 'p'},
                                         {"force-wipe", 0, 0, 'w'},
                                         {"remote", 1, 0, 'r'},
  ...
  while (opt_keep_processing &&
         ((c = getopt_long (argc, argv, "x:p:wlr:c:i:hv", long_options,
                            &option_index)) != -1)) {
    switch (c) {
      case 'x':
  ...
  switch (mode) {
    ...
    case 'i': {
      ...
      int iofds[] = {1, 2, pipefd_in[0], pipefd_command_in[0],
                     pipefd_command_out[1]};
      ...
      char* line;
      for (;;) {  // rl_on_new_line_with_prompt ();
        line = readline ("user=> ");
        if (!line) break;
